...
pub fn render_top10(app: &App) {
    ...
    // ── Tab bar ───────────────────────────────────────────────────────────────
    draw_rectangle_lines(1.0, header_h, sw - 2.0, tab_h, 1.5, DARKGRAY);
    let mut tx = 16.0;
    for (i, label) in TABS.iter().enumerate() {
        ...
        if active {
            draw_rectangle(tx - 2.0, header_h + 2.0, w + 4.0, tab_h - 4.0,
                Color::new(0.0, 0.25, 0.0, 1.0));
            draw_rectangle_lines(tx - 2.0, header_h + 2.0, w + 4.0, tab_h - 4.0, 1.0, GREEN_BBS);
        }
        draw_text_ex(&padded, tx, header_h + CHAR_H * 1.1,
            TextParams { font_size: FONT_SIZE,
                color: if active { GREEN_BBS } else { DIM_GRAY },
                ..Default::default() });
        tx += w + 8.0;
    }
...
fn render_callers(callers: Vec<&crate::bbs::data::TopCaller>, body_y: f32, sw: f32) {
    ...
    draw_line(8.0, body_y + CHAR_H * 1.25, sw - 8.0, body_y + CHAR_H * 1.25, 1.0, DARKGRAY);
